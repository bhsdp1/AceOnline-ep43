...
public:
	...
	BOOL SendAddData(BYTE* pData, int nSize, EN_PACKET_PRIORITY i_enPacketPriority = EN_PACKET_PRIORITY_HIGHEST, BOOL bRawSend = FALSE, BOOL bSessionEnd = FALSE);	// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가 - // 소켓에 전송할 데이타를 설정한다.
	...
	BOOL SendAddRawData(BYTE* pData, int nSize, BOOL bSessionEnd = FALSE);				// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
	...
	virtual BOOL OnRecvdPacketAuthenticationServer(const char* pPacket, int nLength, BYTE nSeq, char* pPeerIP = "", int nPeerPort = 0, SThreadInfo *i_pThreadInfo=NULL);		// 2011-01-26 by hskim, 인증 서버 구현
	virtual BOOL OnRecvdPacketFieldWebServer(const char* pPacket, int nLength, BYTE nSeq, char* pPeerIP = "", int nPeerPort = 0, SThreadInfo *i_pThreadInfo=NULL);				// 2013-03-13 by hskim, 웹 캐시 상점
...
protected:
	...
	CRecvHTTPPacket			m_RecvHTTPPacket;					// 2013-03-13 by hskim, 웹 캐시 상점 - Received HTTP 데이타를 헤더와 바디로 나누기 위한 변수
