// WaveFile.cpp: implementation of the WaveFile class.
//
//////////////////////////////////////////////////////////////////////
#define STRICT
#include "stdafx.h"
...
{
	FLOG( "WaveFile::Open( LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )" );
    HRESULT hr;
...
//-----------------------------------------------------------------------------
HRESULT WaveFile::OpenFromMemory( BYTE* pbData, 
								 ULONG ulDataSize, 
                                 WAVEFORMATEX* pwfx, 
								 DWORD dwFlags )
{
	FLOG( "WaveFile::OpenFromMemory( BYTE* pbData, ULONG ulDataSize, WAVEFORMATEX* pwfx, DWORD dwFlags )" );
    m_pwfx       = pwfx;
...
//       Reset() is called.
//-----------------------------------------------------------------------------
HRESULT WaveFile::Read( BYTE* pBuffer, 
					   DWORD dwSizeToRead, 
					   DWORD* pdwSizeRead )
{
	FLOG( "WaveFile::Read( BYTE* pBuffer, DWORD dwSizeToRead, DWORD* pdwSizeRead )" );
    if( m_bIsReadingFromMemory )
    {
        if( m_pbDataCur == NULL )
            return CO_E_NOTINITIALIZED;
...
{
	FLOG( "WaveFile::WriteMMIO( WAVEFORMATEX *pwfxDest )" );
...
//-----------------------------------------------------------------------------
HRESULT WaveFile::Write( UINT nSizeToWrite, 
						BYTE* pbSrcData, 
						UINT* pnSizeWrote )
{
	FLOG( "WaveFile::Write( UINT nSizeToWrite, BYTE* pbSrcData, UINT* pnSizeWrote )" );
    UINT cT;
