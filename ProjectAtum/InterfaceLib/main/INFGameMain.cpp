...
//	SAFE_DELETE(m_pImgUnitLock) ;
	...
	// 2015-05-01 by killburne
	SAFE_DELETE(m_pStrategicPointDisplay);
...
// 			// end 2012-10-12 by jhjang 해피 아워 경험치 보너스 시스템 리뉴얼
	...
	if (m_pStrategicPointDisplay)
	{
		m_pStrategicPointDisplay->Render();
	}
	// end 2008-04-04 by bhsohn 모선전 관리 클래스 추가
...
//		}		
	...
	else if(dwType == ALPHA_FADE_IN)
	{
		...
		else	// 2015-06-30 Future, fixed unintialized variable
		{
			dwAlpha = 255;
		}
	}
...
BOOL CINFGameMain::FindMotherShipMapIdx(MapIndex_t i_MapIndex, BOOL bCurrent) //0?? ?? , 1?? ?? ???
{
	if (m_pMotherShipC)
...
{/*														
	if(NULL == m_pINFGameMainSysMsg)		//AO 2022 Date and time in top right corner.
	{
		return;
	}
