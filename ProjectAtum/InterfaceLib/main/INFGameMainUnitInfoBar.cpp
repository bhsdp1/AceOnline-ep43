...
#else
	#define	KILLCOUNT_MAX_COUNT					KILLCOUNT_MAX
...
// 2012-11-16 by mspark, 데카의 기운 표시 리뉴얼
#if defined(S_BONUSEXPSYSTEM_RENEWAL) || defined(S_BONUS_KILL_SYSTEM_RENEWAL)
...
#ifdef _WARRIOR_CLASS_KILL_BAR
int nWSWArrClass[] = { 300, 900, 1800, 3000, 4500, 6300, 7400, 9800, 12500, 15500, 19000, 23000, 27500, 32500, 38000, 44000, 50700, 58100, 66200, 75000, 85000, 97000, 111000, 128000, 148000 };
...
#ifdef _WARRIOR_CLASS_KILL_BAR
#define STR_GAGEINFO_FAME "[\\g%d\\g] Class: \\a%s\\a, %5.2f%%"
#define STR_GAGEINFO_FAME_MAX "[\\r%d\\r] Class: \\e%s\\e"
#endif
...
void CINFGameMainUnitInfoBar::SetEXP(float fExp)
{
	...
	{
		sprintf(m_szGageInfo[GAMEMAIN_GAGE_EXP_UP], "Level %d  %5.3f%%", g_pShuttleChild->m_myShuttleInfo.Level, fExp);
	}
#else
...
//	RenderStringBar();							// 2006-06-08 by ispark, 이벤트 설명
	...
	int intpart = (int)fPing;
	float decpart = fPing - intpart;
	int nLatency;
	if (fPing > 0.999f)
	{
		nLatency = 999;
	...
	{
		nLatency = decpart * 1000;
	}
	for (int i = 0; i < 3; i++)
	{
		int nNum = nLatency % 10;
		m_fpsNumbers[nNum]->Move(startX + (m_fpsNumbers[nNum]->GetImgSize().x - 3) *(1 - i) , 160);
		m_fpsNumbers[nNum]->Render();
		nLatency /= 10;
	}
	...
	if(m_fGageRate[GAMEMAIN_GAGE_HP_UP]>0)
	{
		...
		if(i > 0)
		{
			...
			if (g_pSOption->sWSWAddAIMInfo)
			{
				...
				{
					m_pImgMainGageHP[i]->Move(SWFIRE_ANGLE_START_X + (10 * SWTARGET_MOUSE_SCALE*0.6), SWFIRE_ANGLE_START_Y + (10 * SWTARGET_MOUSE_SCALE*0.6));
	...
	if(m_fGageRate[GAMEMAIN_GAGE_DP_UP]>0)
	{
		...
		if(i > 0)
		{
			...
			if (g_pSOption->sWSWAddAIMInfo)
			{
				if ((g_pShuttleChild->m_myShuttleInfo.MapChannelIndex.MapIndex > 2500) && (g_pD3dApp->m_dwGameState == _GAME || g_pD3dApp->m_dwGameState == _CITY || g_pD3dApp->m_dwGameState == _SHOP))
				{
					m_pImgMainGageDP[i]->Move(SWFIRE_ANGLE_START_X + (75 * SWTARGET_MOUSE_SCALE*0.6), SWFIRE_ANGLE_START_Y + (10 * SWTARGET_MOUSE_SCALE*0.6));
...
void CINFGameMainUnitInfoBar::DecaToolTipFunction(BOOL nMode, int nY)	//TRUE면 데카의 축복 FALSE면 데카의 기운 
{
	...
	if(nMode)
		...
		sprintf(buf,STRMSG_C_130627_0002, KILLCOUNT_MAX_COUNT);	// "\\eStrength of Decca\\e: Defeat %d players without logging off to receive a special war reward."
