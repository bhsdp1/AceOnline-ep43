...
// 추가 수정 채팅
	...
	#else
		#define CHAT_MACRO_OUTPUT_TIME		300.0f // 2012-03-30 by isshin 매크로 시스템 버그 수정- 출력 간격 시간
	#endif
...
//						m_nSizeMaxButtonstate = CHAT_BUTTON_DOWN;
			...
			//#ifdef LANGUAGE_CHINA
...
#ifdef _REMEMBER_CHAT_COLOR
				char strLastColorChar = NULL;
				if (strlen(m_strInputMessage) > 3)
				{
					if (m_strInputMessage[0] == '\\' && CheckFontColor(m_strInputMessage[1]))
					{
						strLastColorChar = m_strInputMessage[1];
					}
					else if (m_strInputMessage[1] == '\\' && CheckFontColor(m_strInputMessage[2]))
					{
						strLastColorChar = m_strInputMessage[2];
					}
					else
					{
				...
				if(!m_bChatMode)
				{
					...
					{
						if (g_pD3dApp->IsDoingInfluenceWar())
						{
							g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_CNC_DOING_INFLUENCE_WAR, COLOR_SYSTEM);
							bSendSpecialChat = TRUE;
						}
						else if (g_pD3dApp->GetArenaState() != ARENA_STATE_NONE)
						{
							bSendSpecialChat = TRUE;
						}
						else if (strlen(m_strInputMessage) > 1)
