...
void CSetEventMonsterDlg::OnOK() 
{
	...
	if(FALSE == IS_IN_RANGE(1, m_ctlnMonsterCount, 100))
	{
		AfxMessageBox("Invalid MonsterCount !!");
		return;
	}

	if(FALSE == IS_IN_RANGE(1, m_ctlnDelayTime, MAX_DELAY_TIME_AT_EVENT_MONSTER))
	{
		char szTemp[1024];
	...
	if(FALSE == IS_IN_RANGE(0, m_ctlnProbability, PROB10K_MAX_VALUE + 1))
	{
	...
	m_pEventMonser->SummonProbability		= min(m_ctlnProbability, PROB10K_MAX_VALUE);
