...
CFieldIOCP::~CFieldIOCP()
{
	...
	SAFE_DELETE(m_pDGAEventManager);
// 2008-01-10 by cmkwon, 아이템 이벤트 시스템에 신 쿠폰 시스템 추가 - 삭제됨
...
BOOL CFieldIOCP::InsertFieldServerInfoForArenaServer(SARENA_USE_FIELD_SERVER_INFO * i_pFieldServerInfo)
{
	...
	for(; itr != m_mtvectSARENA_USE_FIELD_SERVER_INFO.end(); itr++)
	{
		if(i_pFieldServerInfo->MFS_ID == itr->MFS_ID
			...
			&& 0 == strcmp(i_pFieldServerInfo->MFS_IP, itr->MFS_IP)
...
{
	// Create Cache Object
	KILLOUNT_CACHE_DATA cacheData = { characterUID, count, ATUM_DATE_TIME(true) };
...
INT CFieldIOCP::GetCachedKillCount(UID32_t characterUID)
{
	...
	KILLOUNT_CACHE_DATA* cacheData = m_mtmapCharacterKillCounts.findLock_Ptr(characterUID);
...
void CFieldIOCP::CleanKillCountCache()
{
	...
	m_mtmapCharacterKillCounts.lock();
	mt_map<UID32_t, KILLOUNT_CACHE_DATA>::iterator itr = m_mtmapCharacterKillCounts.begin();
