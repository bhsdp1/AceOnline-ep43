...
struct SGUILD_DAMAGE
{
	...
	SGUILD_DAMAGE()
	{
		GuildUID1		= 0;
		fSumOfDamage1	= 0;
		MEMSET_ZERO(GuildName2, SIZE_MAX_GUILD_NAME);	// 2007-08-23 by cmkwon, 모선 폭파시 최고 데미지 준 여단명 표시 -
	}
	SGUILD_DAMAGE(UID32_t i_guildUID, char *i_szGName, float i_fDamage)
	{
		GuildUID1		= i_guildUID;
		STRNCPY_MEMSET(GuildName2, i_szGName, SIZE_MAX_GUILD_NAME);	// 2007-08-23 by cmkwon, 모선 폭파시 최고 데미지 준 여단명 표시 -
