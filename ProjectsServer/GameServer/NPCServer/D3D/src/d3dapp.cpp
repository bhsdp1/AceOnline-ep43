...
{
	//const TCHAR* processingTypeNames[] = { "Unknown", "Hardware Vertex processing", "Mixed Vertex processing", "Software Vertex processing" };
	const TCHAR* devTypeNames[] = { "Unknown", "HAL", "Software", "Reference" };
    HRESULT hr;
    TCHAR strMsg[512];
	DWORD processingTypes[] = { D3DCREATE_HARDWARE_VERTEXPROCESSING, D3DCREATE_MIXED_VERTEXPROCESSING, D3DCREATE_SOFTWARE_VERTEXPROCESSING };
	...
	for (int i = 0; i < 3; i++)
	{
		for(int j = 0; j < 3; i++)
		{			
			hr = m_pD3D->CreateDevice(D3DADAPTER_DEFAULT, devTypes[i], hWnd, processingTypes[j] | D3DCREATE_MULTITHREADED, &m_d3dpp, &m_pd3dDevice);

			if( SUCCEEDED(hr) )
			{
				D3DCAPS9 d3dInfo;
				D3DADAPTER_IDENTIFIER9 identifier;
				m_pd3dDevice->GetDeviceCaps(&d3dInfo);
				m_pD3D->GetAdapterIdentifier(D3DADAPTER_DEFAULT, 0, &identifier);
				_stprintf( strMsg, _T("Used Devicetype of %s with Adapter: %s\n"),
								   devTypeNames[i], identifier.Description);
...
// by bergi9 - skip all this shit which is no use for npcserver
/*
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    switch( hr )
    {
		...
		case D3DERR_INVALIDCALL:
            _tcscpy( strMsg, _T("Invalid call. Maybe no DirectX Software Rasterizer (from SDK) installed or D3dref9.dll & co is missing in this folder") );
    		break;
        default:
    		_stprintf( strMsg, _T("Generic application error. Enable\n")
