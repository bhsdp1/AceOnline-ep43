...
{
	vectorObjectInfoServer tmpObjectMonsterInfo;
	vectorObjectInfoServer::iterator itr(m_vectorObjectMonsterInfoCopy.begin());
	for(; itr != m_vectorObjectMonsterInfoCopy.end(); itr++)
	{
		if(itr->m_pMonsterInfo
		...
		{
			tmpObjectMonsterInfo.push_back(*itr);
		}
	}

	g_pNPCGlobal->WriteSystemLogEX(TRUE, "  tmpObjectMonsterInfoSize(%d)\r\n", tmpObjectMonsterInfo.size());

	if(tmpObjectMonsterInfo.size() > 0)
	{
		return &tmpObjectMonsterInfo[rand() % tmpObjectMonsterInfo.size()];
	}
	return NULL;
}
