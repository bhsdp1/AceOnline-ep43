...
void CNPCMapWorkspace::AddMessageMSG_FN_MONSTER_STRATEGYPOINT_SUMMON(MapIndex_t i_MapIndex)
{
	...
	for(; itr != m_vectorPtrMapProject.end(); itr++)
	{
		...
		if(pNMapPro->m_nMapIndex == i_MapIndex)
		{
			...
			pMsg.bVCNMapInflTyforInit = FALSE;	// 2007-09-19 by cmkwon, 기본값
			if(IS_MAP_INFLUENCE_VCN(pNMapPro->m_nMapInfluenceType))	// 2007-10-25 by dhjin, 바이제니유 세력 맵 체크 오류로 수정
			{
				pMsg.bVCNMapInflTyforInit = TRUE;
			}
