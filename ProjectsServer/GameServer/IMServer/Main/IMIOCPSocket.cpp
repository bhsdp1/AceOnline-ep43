...
ProcessResult CIMIOCPSocket::Process_IC_PARTY_CREATE(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	if(pMsgPartyCreate->PartyInfo.MinLevel > pMsgPartyCreate->PartyInfo.MaxLevel
		|| FALSE == IS_IN_RANGE(1, pMsgPartyCreate->PartyInfo.MinLevel, CHARACTER_MAX_LEVEL)
		|| FALSE == IS_IN_RANGE(1, pMsgPartyCreate->PartyInfo.MaxLevel, CHARACTER_MAX_LEVEL)
		)
	{
...
ProcessResult CIMIOCPSocket::Process_IC_PARTY_CHANGE_INFO(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-12-29 by cmkwon, 캐릭터 최대 레벨 상향(110으로) - 
	if(pMsg->PartyInfo.MinLevel > pMsg->PartyInfo.MaxLevel
		|| FALSE == IS_IN_RANGE(1, pMsg->PartyInfo.MinLevel, CHARACTER_MAX_LEVEL)
		|| FALSE == IS_IN_RANGE(1, pMsg->PartyInfo.MaxLevel, CHARACTER_MAX_LEVEL)
		)
	{
...
void CIMIOCPSocket::SendCNC(MSG_IC_CHAT_CNC* msgCnC)
{
	...
	STRNCPY_MEMSET(msgCnC->InfluenceOrStaffPrefix, GetSmallRaceOrInfluenceString(senderSocket->m_character.Race, senderSocket->m_character.InfluenceType), SIZE_MAX_INFLUENCE_OR_STAFFPREFIX);
