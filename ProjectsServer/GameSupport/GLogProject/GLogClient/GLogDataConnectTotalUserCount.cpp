...
{
	MEMSET_ZERO(m_arrConnectTotalUserCountData,sizeof(CConnectTotalUserCountData)*GLOG_DATA_MAX_CNT);
}
...
Err_t CGLogDataConnectTotalUserCount::GLogDBSelect(int * o_pSelectCnt)
{
	...
	while(!m_pSourceRecordSet->EndOfFile)
	{
		...
		MEMSET_ZERO(&tmpConnectTotalUserCountData,sizeof(CConnectTotalUserCountData));
