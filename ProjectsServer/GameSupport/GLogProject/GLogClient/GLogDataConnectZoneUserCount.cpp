...
{
	MEMSET_ZERO(m_arrConnectZoneUserCountData,sizeof(CConnectZoneUserCountData)*GLOG_DATA_MAX_CNT);
}
...
Err_t CGLogDataConnectZoneUserCount::GLogDBSelect(int * o_pSelectCnt)
{
	...
	while(!m_pSourceRecordSet->EndOfFile)
	{
		...
		MEMSET_ZERO(&tmpConnectZoneUserCountData,sizeof(CConnectZoneUserCountData));
