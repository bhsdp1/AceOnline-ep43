...
{
	MEMSET_ZERO(m_arrAceConcurrentUserData,sizeof(CAceConcurrentUserData)*GLOG_DATA_MAX_CNT);
}
...
Err_t CGLogDataAceConcurrentUser::GLogDBSelect(int * o_pSelectCnt)
{
	...
	while(!m_pSourceRecordSet->EndOfFile)
	{
		...
		MEMSET_ZERO(&tmpAceConcurrentUserData,sizeof(CAceConcurrentUserData));
